...
MV.prototype = {
    ...
    _visualize: function(analyser) {
        ...
        gradient.addColorStop(1, 'rgba(100,154,193,0.62)');
        gradient.addColorStop(0.5, 'rgba(31,75,103,0.62)');
        gradient.addColorStop(0, 'rgba(115,184,217,0.62)');
        ctx.fillStyle = gradient;
    ...
    _visualize_flow: function(analyser) {
        ...
        var random = function (m, n) {
            return 1.5 * Math.round(Math.random()*(n - m) + m);
        };
        for (var i = 0; i < num; i++) {
            if(i%3 === 0) {
                var x = random(0, cwidth),
                    y = random(0, cheight),
                    color= 'rgba(187,223,241,0.37)';
            }
            if(i%3 === 1) {
                var x = random(0, cwidth),
                    y = random(0, cheight),
                    color= 'rgba(19,101,145,0.31)';
            }
            if(i%3 === 2) {
                var x = random(0, cwidth),
                    y = random(0, cheight),
                    color= 'rgba(15,63,119,0.3)';
            }
        ...
        var drawMeter = function() {
            ...
            for (var n = 0; n < num; n++) {
                ...
                //能量球半径，与画布大小关联起来
                s.radius = 1.5 * Math.round(array[n * step] / 256 * (cwidth > cheight ? cwidth / 22 : cheight / 16));
                //加了一点模糊发光的效果
...
//				gradient.addColorStop(1, s.color);
    ...
    _audioEnd: function() {
        ...
        this.status = 0;
        document.getElementById('info').innerHTML = 'MP3 visualization';
